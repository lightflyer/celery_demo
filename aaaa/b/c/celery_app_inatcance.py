...
class Config1:
    broker_url = 'redis://127.0.0.1:6379/0'  # 使用redis
    # broker_url = 'sqla+sqlite:///celerydb.sqlite' # 使用sqlite
    # broker_url = 'amqp://guest:guest@localhost:5672'  # 使用rabbitmq
...
if __name__ == '__main__':
    ...
    celery_app.worker_main(
        argv=['worker', '--pool=eventlet', '--concurrency=20', '-n', 'worker1@%h', '--loglevel=debug',
    ...
    celery   worker --app=aaaa.b.c.celery_app_inatcance:celery_app --pool=solo --concurrency=5  --queues=queue_add,queue_sub
    """
